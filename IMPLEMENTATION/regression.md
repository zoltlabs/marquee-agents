...
- If `source` is missing, interactively ask for it.
...
- Check if `target_dir` has `filelist.txt`. If yes, use it. Else, ask the user to use the bundled default from `qa-agent` scripts. If yes, copy it. If no, exit with a message.
